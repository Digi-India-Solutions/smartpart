...
const ProductCard = ({ image = '', name = '', description = '', part_no = '', id = '', product }) => {
    // const imgSrc = image?.startsWith('uploads/product')
    //   ? `${serverURL}/${image}`
    ...
    return (
        <div className="AllProduct-card">
            ...
            <div className="Allproduct-info">
                <h3>{name}</h3>
                <div className="Allproduct-details">{safeHtmlParse(description)}</div>
                <p className="Allproduct-part">
...
const Page = ({ id = "" }) => {
    ...
    const loadInitialFilters = async () => {
        await Promise.all([
            ...
            fetchData(`brand/get-all-brand-by-id/${searchTerm}`, setBrandTop, "Failed to load brand")
        ]);
    };
    ...
    const handleToggle = (filterType, id) => {
        ...
        if (filterType === "brandCategory") {
            setFilters(prev => ({ ...prev, selectedBrandCategory: updateSelection(prev.selectedBrandCategory) }));
    ...
    const renderPagination = () => {
        ...
        return (
            <div className="pagination d-flex justify-content-center gap-2 flex-wrap mb-5">
                ...
                {pages.map((page) => (
                    <button key={page} className={`btn ${page === currentPage ? "btn-light text-dark" : "btn-outline-light"}`} onClick={() => setCurrentPage(page)}>{page}</button>
                ))}
        ...
        <>
            ...
            {id && title !== 'category' && (
                <section className="DynamicProductSec bg-black pt-3">
                    <div className="container">
                        <h2 className="mb-4 text-center">
                            All Products of <span className="text-theme bouncing-text">{brandTop?.name}</span>
                        </h2>
                        <div className="row">
                            <div className="col-md-12 mb-5">
                                <div className="card h-100 shadow-sm">
                                    <div className="blogingSec">
                                        <div className="card-body">
                                            <h3 className="card-title text-center text-primary">{brandTop?.top_title}</h3>
                                            <p className="card-text" style={{ whiteSpace: "pre-line" }}>{brandTop?.top_des}</p>
                                            <Link href="/pages/contact-us" className="btn btn-outline-primary mt-3">Continue Reading →</Link>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div className="row">
                        <div className="col-lg-3 col-md-4">
                            <aside className="filters p-3 bg-dark rounded" style={{ marginTop: 40 }}>
                                ...
                                />
                                <div className="mt-3">
                                    <button className="btn btn-primary w-100" onClick={() => fetchProducts()}>Filter Data</button>
                                </div>
                            </aside>
                        </div>
